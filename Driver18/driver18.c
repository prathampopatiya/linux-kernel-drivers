 ...
 * locked or not. 
 *
 *  Spin Lock:- in mutex thread tries to acquire lock. but if it's not available then it goes to sleep 
 *  but in spin lock it's different if process attempts to acquire spin lock and its unavailable 
 *  then keeps trying continuously(spinning) till it get's the lock 
 *
 *  DEFINE_SPINLOCK(etx_spinlock); The macro given above will create a spinlock_t variable in the name of 
 *  etx_spinlock and initialize to UNLOCKED STATE.
 *  
 ...
 *  
 *  spin_lock(spinlock_t *lock); This will take the lock if it is free, otherwise, 
 *  it’ll spin until that lock is free (Keep trying). 
 *
 *  spin_trylock(spinlock_t *lock); Locks the spinlock if it is not already locked. 
 *  If unable to obtain the lock. 
 *  
 *  spin_unlock(spinlock_t *lock);  It will unlock which is locked by the above call. 
 *
 *  Antoher Approach:
 *  If you share data with a bottom half and user context (like Kernel Thread), 
 *  then this approach will be useful.
 *
 *  spin_lock_bh(spinlock_t *lock); disables soft interrupts on that CPU, then grabs the lock. 
 *  This has the effect of preventing softirqs, tasklets, and bottom halves from running on the local CPU.
 *  spin_unlock_bh(spinlock_t *lock); used to unlock the lock acquired by the above method 
 *
 *  spin_lock_irq(spinlock_t *lock); this blocks hardware interrupts caused by irq's if sharing data b/w 
 *  ISR is being done bottom halves can be interrupted by hardware interrupts.  
