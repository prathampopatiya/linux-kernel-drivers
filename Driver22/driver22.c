...
enum hrtimer_restart timer_callback(struct hrtimer *timer){
    ...
    // Timer stuff
    pr_info("[*] Time callback func called: %d\n",count++);
...
static int __init etx_driver_init(void){
    ...
    hrtimer_setup(&etx_hr_timer,timer_callback,CLOCK_MONOTONIC,HRTIMER_MODE_REL);
...
MODULE_DESCRIPTION("High Resolution Timers in Linux kernel drivers");
MODULE_VERSION("1.18");
