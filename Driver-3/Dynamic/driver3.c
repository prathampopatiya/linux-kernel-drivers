...
// Creating a device with our choose major and minor number wirh MKDEV macro for static
dev_t dev = 0; // dynamic
...
static int __init hello_world_init(void){
    ...
    */
    if((alloc_chrdev_region(&dev,0,1,"DynamicDevv")) < 0){
        pr_info("[-] Cannot allocate major number to device 1\n");
        return -1;
    }
...
MODULE_DESCRIPTION("Simple linux driver dynamically allocating minor and major number for device");
MODULE_VERSION("1.0");
