...
static long etx_ioctl(struct file *file,unsigned int cmd,unsigned long arg){
    switch (cmd) {
        case WR_VALUE:
            ...
            pr_info("[+] Data Write successfull Value: %d\n",value);
            break;
        case RD_VALUE:
...
static int __init etx_driver_init(void){
    ...
    if(IS_ERR(dev_class = class_create("etx_class_device"))){
        pr_err("[-] Error creating class\n");
        goto r_class;
    }
    // Create the device 
    if(IS_ERR(device_create(dev_class,NULL,dev,NULL,"etx_ioctl_device"))){
